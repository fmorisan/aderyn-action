...
install_solc()
{
    if [[ -z "$SOLCVER" ]]; then
        ...
        if [[ -f "$TARGET" ]]; then
            ...
            popd >/dev/null
        else echo "[-] Target is neither a file nor a directory, assuming it is a path glob"
...
if [[ -z "$SLITHERARGS" ]]; then
    $HOME/.cyfrin/bin/aderyn "$TARGET" | tee "$STDOUTFILE"
else
    echo "[-] SLITHERARGS provided. Running slither with extra arguments"
    printf "%s\n" "$SLITHERARGS" | xargs $HOME/.cyfrin/bin/aderyn "$TARGET" $SARIFFLAG $IGNORECOMPILEFLAG $FAILONFLAG $CONFIGFLAG | tee "$STDOUTFILE"
fi
